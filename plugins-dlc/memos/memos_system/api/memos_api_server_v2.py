...
# 设置 HuggingFace 缓存路径和镜像
import os
os.environ.setdefault('HF_HOME', os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'huggingface_cache'))
...
async def startup_event():
    ...
    try:
        ...
        # 2. 加载 Embedding 模型（仅在 use_api=false 时需要本地模型）
        use_api_embedding = config.get('embedding', {}).get('use_api', False)
        if not use_api_embedding:
            ...
            try:
                ...
                print("[跳过] sentence_transformers 未安装，使用 API 嵌入模式")
            except Exception as e:
                print(f"[警告] Embedding 模型加载失败: {e}")
        else:
            print("[OK] 使用 API 嵌入模式（无需本地模型）")
...
async def search_memory(request: SearchMemoryRequest):
    ...
    try:
        ...
        results = [r for r in results if r.get('similarity', 0) >= threshold or r.get('bm25_only', False)]
