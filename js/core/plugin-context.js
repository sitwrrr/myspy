...
class PluginContext {
    ...
    async callLLM(prompt, options = {}) {
        ...
        // 过滤 think/reasoning 等思考内容
        content = content.replace(new RegExp('<think>[\\s\\S]*?<\\/think>', 'gi'), '');
        content = content.replace(new RegExp('<reasoning>[\\s\\S]*?<\\/reasoning>', 'gi'), '');
        return content.trim();
    }
    ...
    speakText(text) {
        ...
        voiceChat.ttsProcessor.speak(text, global.modelController);
    }
